...
def test_grep_performance():
    ...
    measure("grep_design_pattern", grep_tool.execute, ctx, path=GIGANTIC_FILE,
            pattern="class (Singleton|Factory|Builder|Observer|Strategy|"
                    "Decorator|Adapter|Proxy|Command|StateMachine|ChainOfResponsibility)")
    measure("grep_data_structure", grep_tool.execute, ctx, path=GIGANTIC_FILE,
            pattern="class (ListNode|LinkedList|TreeNode|BinaryTree|TrieNode|Trie|Graph)")
...
def main():
    ...
    with open(GIGANTIC_FILE, encoding='utf-8') as f:
